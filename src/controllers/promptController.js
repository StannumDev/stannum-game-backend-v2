...
const getAllPrompts = async (req, res) => {
    try {
        ...
        if (search && search.trim().length >= 2) filters.$text = { $search: search.trim() };
        let query = Prompt.find(filters).populate('author', 'username profile.name preferences.hasProfilePhoto').sort(sortConfig);
        ...
        let promptsWithUserActions = prompts;
        if (req.userAuth) {
            promptsWithUserActions = prompts.map(prompt => prompt.getPreview(req.userAuth.id));
        } else {
            promptsWithUserActions = prompts.map(prompt => prompt.getPreview());
        }
...
const getPromptById = async (req, res) => {
    try {
        ...
        const promptDetails = req.userAuth ? prompt.getFullDetails(req.userAuth.id) : prompt.getFullDetails();
...
const likePrompt = async (req, res) => {
  try {
    ...
    return res.json({
        success: true,
        data: {
            likesCount: prompt.metrics.likesCount
        },
...
const toggleFavorite = async (req, res) => {
    try {
        ...
        const isFavorited = user.hasPromptInFavorites(id);
        if (isFavorited) {
            await Promise.all([
                user.removePromptFromFavorites(id),
                prompt.removeFavorite(req.userAuth.id)
            ]);
            ...
            return res.json({
                ...
                data: {
                    isFavorited: false,
                    favoritesCount: prompt.metrics.favoritesCount
                },
                message: 'Removed from favorites'
            });
        } else {
            await Promise.all([
                user.addPromptToFavorites(id),
                prompt.addFavorite(req.userAuth.id)
            ]);
...
const getUserPrompts = async (req, res) => {
    try {
        ...
        const promptsData = prompts.map(prompt => req.userAuth ? prompt.getPreview(req.userAuth.id) : prompt.getPreview());
...
const getTopPrompts = async (req, res) => {
    try {
        ...
        const promptsData = topPrompts.map(prompt => req.userAuth ? prompt.getPreview(req.userAuth.id) : prompt.getPreview());
...
module.exports = { getAllPrompts, getPromptById, createPrompt, deletePrompt, copyPrompt, likePrompt, unlikePrompt, toggleFavorite, getUserPrompts, getMyPrompts, getMyFavorites, getStats, getTopPrompts };