    ...
    tags: {
        type: [{
            ...
            validate: {
                ...
                message: "Tags can only contain lowercase letters, numbers, hyphens, underscores, dots and spaces"
            }
        }],
        validate: {
...
promptSchema.index({ status: 1, visibility: 1 });
...
promptSchema.index({ 'stannumVerified.isVerified': 1 });
...
promptSchema.methods.addFavorite = function(userId) {
    if (this.favoritedBy.includes(userId)) return Promise.reject(new Error('Prompt already in favorites'));
...
promptSchema.methods.removeFavorite = function(userId) {
    ...
    if (index === -1) return Promise.reject(new Error('Prompt not in favorites'));
...
promptSchema.methods.softDelete = function() {
    this.status = false;
    return this.save();
};

promptSchema.methods.hide = function() {
    if (this.visibility === 'published') {
        this.visibility = 'hidden';
        return this.save();
    }
    return Promise.reject(new Error('Prompt is not published'));
};

promptSchema.methods.publish = function() {
    if (this.visibility !== 'published') {
        this.visibility = 'published';
        return this.save();
    }
    return Promise.reject(new Error('Prompt is already published'));
};

promptSchema.methods.setDraft = function() {
    this.visibility = 'draft';
    return this.save();
};

promptSchema.methods.verifyByStannum = function() {
    this.stannumVerified.isVerified = true;
    this.stannumVerified.verifiedAt = new Date();
    return this.save();
};

promptSchema.methods.removeStannumVerification = function() {
