const getAchievements = (programs) => {
    const flatModulesOf = (prog) => (prog?.sections || []).flatMap(s => s.modules || []);
    ...
    return [
        ...
        },
        {
            ...
            condition: (user) => {
                return programs.some(programCfg => {
                    ...
                    return flatModulesOf(programCfg).some(module => {
        ...
        },
        {
            ...
            condition: (user) => {
                return programs.some(programCfg => {
                    ...
                    return flatModulesOf(programCfg).some(module => {
                        return (module.instructions.length > 0 && module.instructions.every(inst => (userProgram.instructions || []).some(i => i.instructionId === inst.id && i.status === "GRADED")));
                    });
                });
            }
        },
        {
            ...
            condition: (user) => {
                return programs.some(programCfg => {
                    ...
                    return flatModulesOf(programCfg).every(module => {
        ...
        },
        {
            ...
            condition: (user) => {
                ...
                const firstModule = flatModulesOf(tiaProgramCfg)[0];
        ...
        },
        {
            ...
            condition: (user) => {
                ...
                return flatModulesOf(tiaProgramCfg).every(module =>
        ...
        },
        {
            ...
            condition: (user) => {
                ...
                const tiaSummer = user.programs?.tia_summer;
                if (!tiaSummer) return false;
                return flatModulesOf(tiaSummerCfg).every(module => module.lessons.every(lesson => (tiaSummer.lessonsCompleted || []).some(lc => lc.lessonId === lesson.id) ) && ( (module.instructions || []).length === 0 || module.instructions.every(inst => (tiaSummer.instructions || []).some(i => i.instructionId === inst.id && i.status === "GRADED"))));
            }
        },
        {
        ...
        },
        {
            ...
            condition: (user) => {
                ...
                const tiaPool = user.programs?.tia_pool;
                if (!tiaPool) return false;
                return flatModulesOf(tiaPoolCfg).every(module => module.lessons.every(lesson => (tiaPool.lessonsCompleted || []).some(lc => lc.lessonId === lesson.id) ) && ( (module.instructions || []).length === 0 || module.instructions.every(inst => (tiaPool.instructions || []).some(i => i.instructionId === inst.id && i.status === "GRADED"))));
            }
        },
    ];
};
