...
router.get(
    ...
    [
        validateJWT,
        check("username", "El nombre de usuario es inválido.").trim().escape().customSanitizer(value => value.replace(/\s+/g, ' ')).not().isEmpty().withMessage("El nombre de usuario no puede estar vacío.").isLength({ min: 3, max: 30 }).withMessage("El nombre de usuario debe tener entre 3 y 30 caracteres.").matches(/^[a-zA-Z0-9._]+$/).withMessage("El nombre de usuario solo puede contener letras minúsculas, números, puntos y guiones bajos."),
        fieldsValidate,
    ],
