...
router.post(
  ...
  [
    ...
    check("message", "El diagnóstico es obligatorio.").trim().customSanitizer(value => value.replace(/<[^>]*>?/gm, '')).notEmpty().withMessage("El diagnóstico no puede estar vacío.").isLength({ min: 50, max: 5000 }).withMessage("El diagnóstico debe tener entre 50 y 5000 caracteres."),
